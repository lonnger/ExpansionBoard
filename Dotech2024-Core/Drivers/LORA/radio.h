...
#include "sx1280.h"
  ...
  struct Radio_s
  {
     ...
     *
     * \param [in]  modParams     A structure describing the modulation
     * parameters
     */
    ...
    /*!
     * \brief Sets the Sync Word given by index used in GFSK, FLRC and BLE
     * protocols
     *
     ...
     *
     * \param [in]  timeout       Structure describing the reception timeout
     * value
     */
     ...
     * radio = SX1280( mosi, miso, sclk, nss, busy, int1, int2, int3, rst,
     * &callbacks ); radio.Init( ); radio.SetPollingMode( );
     *
     * while( true )
     * {
     ...
     * // Initializations and callbacks declaration/definition
     * radio = SX1280( mosi, miso, sclk, nss, busy, int1, int2, int3, rst,
     * &callbacks ); radio.Init( ); radio.SetInterruptMode( );   // Optionnal.
     ...
     * Activity Detection operations [LORA_CAD_01_SYMBOL, LORA_CAD_02_SYMBOL,
    ...
    /*!
     * \brief Sets the chip to automatically send a packet after the end of a
     * packet reception
     *
    ...
    /*!
     * \brief Stop the chip from automatically sending a packet after the end of
     * a packet reception if previously activated with SX1280::SetAutoTx command
     *
     * \see SX1280::SetAutoTx
     */
    void (*StopAutoTx)();
    /*!
     * \brief Sets the chip to automatically receive a packet after the end of a
     * packet transmission
     *
     * \remark The offset is automatically compensated inside the function
     *
    ...
    /*!
     * \brief Sets the number of bits used to check that ranging request match
     * ranging ID
     *
     ...
     *
     * \retval      ranging       The ranging measure filtered according to
     * resultType [m]
     */
     ...
     * must be re-performed for each new SX1280 PCB design. The value is
     * obtained empirically by either conducted measurement in a known
     * electrical length coaxial RF cable (where the design is connectorised) or
     * by radiated measurement, at a known distance, where an antenna is
     * present. The result of the calibration process is that the SX1280 ranging
     * result accurately reflects the physical range, the calibration procedure
     * therefore removes the average timing error from the time-of-flight
     * measurement for a given design.
     *
    ...
    /*!
     * \brief Return the Estimated Frequency Error in LORA and RANGING
     * operations
     *
